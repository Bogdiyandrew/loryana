...
export default function HomePage() {
  ...
  const unlockDate = new Date("2026-02-13T22:30:00") 
  ...
  const aboneazaMa = async () => {
    if (typeof window !== "undefined") {
      console.log("🔔 Încerc abonarea manuală...");
      try {
        // Forțăm apariția ferestrei
        await OneSignal.Slidedown.promptPush();
      } catch (error) {
        console.error("Eroare la deschiderea prompt-ului:", error);
        alert("Eroare: " + error);
      }
    }
  };
